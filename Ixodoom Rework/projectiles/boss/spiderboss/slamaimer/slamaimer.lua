...
function destroy()
  if projectile.sourceEntity() and world.entityExists(projectile.sourceEntity()) then
    ...
    world.spawnProjectile("spiderslam", mcontroller.position(), projectile.sourceEntity(), {math.cos(rotation), math.sin(rotation)}, false, { speed = 20, power = projectile.getParameter("power")})
