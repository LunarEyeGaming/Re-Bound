...
function MeleeCombo:windup()
  ...
  if self.comboStep > 1 then
    preStance = self.stances.dynamicWait
  else
    preStance = self.stances.idle
  end
...
function MeleeCombo:wait()
  ...
  local postStance = self.stances.idle
  ...
  util.wait(stance.duration, function()
    if stance.shouldInterpolate then
      RBWepUtil.lerpStance(stance, postStance, progress)
      progress = math.min(1.0, progress + (self.dt / stance.duration))
    end
    if self:shouldActivate() then
      self.stances.dynamicWait = RBWepUtil.getLerpStance(stance, postStance, progress)
      self:setState(self.windup)
      return
    end
  end)
...
function MeleeCombo:preslash()
  ...
  local preStance = self.stances["windup"..self.comboStep]
...
function MeleeCombo:fire()
  ...
  if self.comboStep < self.comboSteps then
    ...
    if stance.shouldInterpolateEnd then
      local postStance = self.stances.idle
      RBWepUtil.animateStance(stance, postStance, stance.resetDuration, self.dt)
    end
  end
end
